...
class TestElement {
    ...
    fun testAddAttribute() {
        ...
        assertEquals(Attribute("name", "Mestrado em Engenharia Informática", curso), curso.findAttribute("name"))
    ...
    fun testRemoveAttribute() {
        ...
        assertEquals(null, curso.findAttribute("name"))

        val exception = assertThrows<NotFoundException> {
            curso.removeAttribute("avaliacao")
        }
        assertEquals("avaliacao attribute not found.", exception.message)
    ...
    fun testChangeAttribute() {
        ...
        assertEquals("Mestrado", curso.findAttribute("name")?.value)

        val exception = assertThrows<NotFoundException> {
            curso.changeAttribute("nome", "Mestrado")
        }
        assertEquals("nome attribute not found.", exception.message)
    ...
    fun testRenameAttribute() {
        ...
        assertEquals(Attribute("nome", "Mestrado em Engenharia Informática", curso), curso.findAttribute("nome"))


        val exception = assertThrows<NotFoundException> {
            curso.renameAttribute("nme", "mestrado")
        }
        assertEquals("nme attribute not found.", exception.message)


        val exception1 = assertThrows<InvalidNameException> {
            curso.renameAttribute("nome","na#ds")
        }
        assertEquals("Please introduce a valid name for the attribute.", exception1.message)

        val exception2 = assertThrows<InvalidNameException> {
            curso.renameAttribute("nome","")
        }
